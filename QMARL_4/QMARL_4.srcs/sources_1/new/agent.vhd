...
--USE work.fixed_float_types_c.all;
...
entity agent is
    ...
    Port    (   clk:    in std_logic;
                ...
                al_r:       out std_logic_vector(15 downto 0));
end agent;

architecture Behavioral of agent is
    
    type state_type is (st0_reset, st1_idle, st2_valid, st3_pdf, st4_cumsum, st5_done, stx_alfr);
...
begin
    ...
    te_2 <= To_std_logic_vector(temp2(-9 downto -24));
    al_r <= To_std_logic_vector(alfrew(-9 downto -24));
    o_state <= "000" when state = st0_reset else
               "001" when state = st1_idle  else
               "010" when state = stx_alfr else
               "011" when state = st2_valid else
               "100" when state = st3_pdf else
               "101" when state = st4_cumsum else
               "110" when state = st5_done else
               "111";    
    ...
    begin 
        if rising_edge(clk) then  
            if state = st0_reset then
                ...
                d <= '0';
            elsif state = stx_alfr then
                e_fifo <= '1';
                if alf = 0 then
                    alfrew <= to_sfixed(reward, -7, -22);
                elsif alf = 1 then
                    alfrew <= to_sfixed(reward, -9, -24);
                end if;  
                d <= '0';
