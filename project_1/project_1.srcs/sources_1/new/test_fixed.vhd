...
architecture Behavioral of test_fixed is
    ...
    type state_type is (st0_reset, st1_idle, st2_alfr, st3_valid, st4_pdf, st5_done);
...
begin    
    one <= to_sfixed(1,1,0);
    rw  <= to_sfixed(signed(reward), -9, -24);
    ...
    o_st <= "000" when state = st0_reset else
            ...
            "011" when state = st3_valid else
            "100" when state = st4_pdf   else
